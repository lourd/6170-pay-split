...
class PurchaseTest < ActiveSupport::TestCase
  ...
  test "does not save with purchase amount higher then MAX_PURCHASE_AMOUNT" do
    event = events(:FirstEvent)
    user = users(:A)
    event.update_attribute(:closed, true)
    purchase = Purchase.new(:amount => Purchase::MAX_PURCHASE_AMOUNT + 5, :description => "test",
        :event_id => event.id, :user_id => user.id)
